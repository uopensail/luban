...
void merge_file(std::ofstream &writer, std::string filename)
{
    ...
    std::cout << "Finish Merging File: " << filename << " Total Line Count: " << count << std::endl;
    reader.close();
    remove(filename.c_str());
}

void thread_func(std::string config_file, std::string input_file, std::string output_file)
{
    ...
    while (getline(reader, line))
    {
        ...
        if (count % 1000 == 0)
        {
            std::cout << "Reading File: " << input_file << " Line No." << count << std::endl;
        }
        tensorflow::Example example;
        if (google::protobuf::util::JsonStringToMessage(line, &example, opts).ok())
        {
    ...
    std::cout << "Finish Processing File: " << input_file << " Total Line Count: " << count << std::endl;
    reader.close();
    remove(input_file.c_str());
    writer.close();
}

void process(std::string config_file, std::string input_file, std::string out_file, int num)
{
    split_file(input_file, num);
    std::thread threads[num];
    for (int i = 0; i < num; i++)
    {
        threads[i] = std::thread(thread_func, config_file, input_file + "_" + std::to_string(i),
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "c:i:o:n:")) != -1)
    {
        ...
        case 'n':
            num = std::atoi(optarg);
        default:
            std::cout << "./luban_tool -c config_file -i input_file -o output_file -n thread_num" << std::endl;
        }
    }
